...
export function createServer() {
  const server = new Server(
    { name: 'westron-mcp', version: '0.1.0' },
    { capabilities: { tools: {} } },
  );
...
// Only start the stdio loop when executed directly, so tests can import this file.
const invokedDirectly =
  process.argv[1] && path.resolve(process.argv[1]) === path.resolve(new URL(import.meta.url).pathname);
if (invokedDirectly) {
  main().catch((err) => {
    // stderr only — stdout is the MCP transport.
    console.error(`westron-mcp failed to start: ${err.message}`);
    process.exit(1);
  });
}
